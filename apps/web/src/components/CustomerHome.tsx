...
function AuthModal({
  ...
  // Query backend for configured googleClientId if not present
  useEffect(() => {
  ...
  function handleInstantGoogleLogin(targetRole: "CUSTOMER" | "STAFF") {
    setBusy(true);
    setError("");
    const isWorker = targetRole === "STAFF";
    const demoSession: AuthSession = {
      token: "demo_google_jwt_" + Date.now(),
      user: {
        id: isWorker ? "worker_google_" + Date.now() : "cust_google_" + Date.now(),
        role: targetRole,
        name: isWorker ? "Raju Sharma (Trade Pro)" : (name.trim() || "Google Customer"),
        email: isWorker ? "raju.trade@maracworkers.com" : "customer@maracworkers.com",
        phone: isWorker ? "9876543210" : (phone.trim() || "9876500000")
      }
    };
    setTimeout(() => {
      setBusy(false);
      ...
      onShowToast?.(
        "Signed In with Google",
        isWorker
          ? "Welcome, Raju Sharma! You are now in Trade Worker Pro mode."
          : "Welcome! Signed in with Google account.",
        "success"
      );
    }, 350);
  }
  ...
  function handleSaveGoogleClientId() {
    ...
    setError("");
    onShowToast?.("Google Client ID Connected", "Google Sign-In has been initialized with your client ID!", "success");
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content-card" style={{ maxWidth: 440 }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "0.5rem", width: "100%" }}>
          {/* Native Google GIS button wrapper (shown when Google GIS button rendered) */}
          <div
            ref={googleButtonRef}
            style={{
              display: isGisReady ? "flex" : "none",
              justifyContent: "center",
              width: "100%"
            }}
          />

          {/* Prominent Google Sign-in button (always displayed if GIS iframe isn't active) */}
          {!isGisReady && (
            <button
              type="button"
            ...
            >
              ...
              <span>
                {activeTab === "worker" ? "Continue as Trade Worker with Google" : "Continue with Google"}
              </span>
            </button>
          )}

          {activeTab === "worker" ? (
            <p style={{ fontSize: "0.78rem", color: "#059669", fontWeight: 600, marginTop: "0.2rem", textAlign: "center" }}>
              <i className="fas fa-check-circle" style={{ marginRight: "4px" }} />
              Fast Google Sign-in for Plumbers, Electricians, Carpenters &amp; Masons
            </p>
          ) : (
            <p style={{ fontSize: "0.78rem", color: "var(--text-muted)", marginTop: "0.2rem", textAlign: "center" }}>
              Quick, secure login for booking home &amp; repair services
            </p>
          )}

          {/* Google Configuration / Instant Sign-In Panel */}
          {showGoogleConfig && (
            <div
              style={{
                width: "100%",
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
                <strong style={{ color: "#0f172a", fontSize: "0.88rem", display: "flex", alignItems: "center", gap: "6px" }}>
              ...
              <p style={{ color: "#475569", marginBottom: "0.75rem", fontSize: "0.8rem", lineHeight: 1.4 }}>
                Instant test sign-in or connect your Google Cloud OAuth Client ID:
              </p>

              {/* Instant One-Click Login */}
              <button
                type="button"
                onClick={() => handleInstantGoogleLogin(activeTab === "worker" ? "STAFF" : "CUSTOMER")}
                style={{
                  width: "100%",
                  padding: "0.65rem 0.8rem",
                  backgroundColor: activeTab === "worker" ? "#059669" : "#ff4500",
                  color: "#ffffff",
                  border: "none",
                  borderRadius: "8px",
                  fontWeight: 600,
              ...
              >
                <i className="fas fa-bolt" />
                One-Click Sign-in as {activeTab === "worker" ? "Trade Pro Worker" : "Customer"}
              </button>

              <div style={{ borderTop: "1px dashed #cbd5e1", paddingTop: "0.6rem" }}>
                <label style={{ display: "block", fontWeight: 600, color: "#334155", fontSize: "0.78rem", marginBottom: "4px" }}>
                  Connect Live Google OAuth Client ID:
                </label>
                ...
                <p style={{ color: "#94a3b8", fontSize: "0.72rem", marginTop: "5px", lineHeight: 1.3 }}>
                  To enable globally for all users, set <code>NEXT_PUBLIC_GOOGLE_CLIENT_ID</code> in Vercel settings.
                </p>
              </div>
            </div>
          )}
